...
class Menu extends \kartik\tree\models\Tree
{
    ...
    public function getUrl($params = [])
    {
        ...
        if ($model->url_type === 'news') $menuParams = ['newsid/index', 'slug' => $model->url_value];
        if ($model->url_type === 'interactive_services') $menuParams = $model->url_value;
        if ($model->url_type === 'category') $menuParams = ['news-category/index', 'id' => $model->url_value];
        if ($model->url_type === 'c-action') $menuParams = [$model->url_value];
        if ($model->url_type === 'leaders') $menuParams = ['leadersid/index', 'slug' => $model->url_value];
